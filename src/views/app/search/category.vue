<template>
  <div class="w-full relative container">
    ...
    <!-- <div class="grid lg:grid-cols-9 gap-4 my-6" v-show="!loading">
      <div class="col-span-12 lg:col-span-2">
        <Filters
          :filters="filters"
          v-model="selected_filters"
    ...
    <div class="text-3xl text-center" v-show="loading">loading ...</div> -->
  </div>
</template>
...
watch(
  ...
  (val, oldVal) => {
    ...
    if (val.range.applied) {
      params.append(`max_price`, val.range.max);
      params.append(`min_price`, val.range.min);
    }
