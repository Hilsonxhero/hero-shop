<template>
  <div class="mt-16">
    <section class="flex flex-col space-y-3 lg:items-center lg:space-y-0 lg:flex-row lg:space-x-reverse"></section>
    <div eager="">
        ...
        <div class="flex w-full items-start">
          <section
            ...
            <div class="w-full mt-1 lg:mt-0">
              <div class="h-full">
                <hx-form :model="form" ref="formRef" class="h-full">
                  <div class="space-y-3 lg:space-y-3">
                    <hx-form-group label="پرسش خود را مطرح کنید">
                      <!-- <hx-textarea v-model="form.content" placeholder="بنویسید ..."></hx-textarea> -->
                      <hx-form-item v-slot="{ field }" prop="content" :rules="[{
                        required: true, message: ' متن پرسش الزامی می باشد'
                      }]" label="عنوان" class="col-span-12">
                        <hx-input class="h-28" type="textarea" v-bind="field" v-model="form.content"
                          placeholder="بنویسید .."></hx-input>
                      </hx-form-item>
                    </hx-form-group>
                    <div class="flex flex-col space-y-3 lg:space-y-4">
                      <hx-button @click="create()" bock>ثبت پرسش</hx-button>
                    </div>
                  </div>
                </hx-form>
              </div>
          ...
          <div class="flex flex-col flex-grow" style="">
            <section class="flex w-full lg:items-center mb-4 mr-4">
              <div class="t-row justify-between">
                <span class="block p-detail-heading p-detail-heading-border">پرسش های کاربران</span>
                <i color="#666" name="t-icon-sort-24px"
                  class="block text-4xl text-typo-light ml-4 mr-4 lg:mr-32 material-icons t-icon-sort-24px"></i>
              </div>
              <span
                class="block cursor-pointer px-2 py-2 lg:py-2 lg:px-4 rounded-xl lg:flex lg:items-center lg:h-[3rem] text-sm">
                جدیدترین ها </span><span
            ...
            <div>
              <div class="lg:mr-4 flex-grow gap-4 grid grid-cols-1">
                <div class="flex w-full" v-for="(question, i) in questions" :key="i">
                  <div
                      ...
                      <section
                        ...
                        <section class="space-y-2 w-full border-b pb-2 border-gray-300">
                          <div class="flex justify-between lg:justify-start">
                            <span class="text-xs font-medium lg:ml-4">{{ question.username }}</span>
                            <div class="text-typo-light text-xs lg:text-sm">{{ question.created_at }}</div>
                          </div>
                          <div class="flex justify-between items-end">
                            ...
                            <section
                              ...
                              <i color="#666" name="t-icon-reply_black_24dp"
                                class="text-3xl ml-4 transition-all opacity-0 group-hover:opacity-100 material-icons t-icon-reply_black_24dp"></i>
                              <span class="text-xs font-medium cursor-pointer"
                                @click="handleOpenAnswerDialog(question)">پاسخ به این پرسش</span>
                            </section>
                          </div>
                        </section>
                      </section>
                    ...
                    <div class="flex mb-3" v-for="(reply, i) in question.replies" :key="i">
                      <div
                      ...
                      <div
                        ...
                        <i color="#666" name="t-icon-reply_black_24dp"
                          class="text-3xl ml-4 text-typo-lighter lg:hidden material-icons t-icon-reply_black_24dp"></i>
                        <section class="space-y-3 lg:space-y-5 w-full h-full flex-col justify-between">
                          <div class="t-row justify-between lg:justify-start">
                            <span class="text-sm lg:font-medium lg:ml-4"> کاربر فالکون</span>
                            <span class="text-typo-light text-xs lg:text-sm">{{ reply.created_at }}</span>
                          </div>
                          <div class="flex flex-col items-end space-y-3 lg:space-y-0 lg:flex-row lg:justify-between">
                            ...
                            <section
                              ...
                              <div class="flex items-center cursor-pointer space-x-reverse space-x-3">
                    ...
                    <div class="t-flex-center py-4 lg:hidden text-link">
                      <i color="#666" name="t-icon-reply_black_24dp"
                        class="text-xl ml-4 material-icons t-icon-reply_black_24dp"></i>
                      <div>پاسخ شما</div>
                    </div>
                  </div>
                </div>
              ...
              <div class="rounded-xl bg-white vue-portal-target">
                <div class="w-full overflow-x-auto t-pagination text-typo-light lg:block" style="" pagecount="1"
                  currentpage="1">
                  <section class="flex justify-between py-3 text-gray-700"></section>
                </div>
              </div>
    ...
    <hx-dialog title="پاسخ به پرسش" width="50%" v-model="visiable_answer_dialog">
      <div class="mx-4">
        <hx-form ref="answerFormRef" :model="answer_form" labhx-width="100px"
          ...
          <div class="w-full flex col-span-12">
            ...
            <section class="group flex items-end justify-between p-3 lg:px-6 lg:py-4 lg:flex-grow w-full lg:w-auto">
              <section class="space-y-2 w-full border-b pb-2 border-gray-300">
                <div class="flex justify-between lg:justify-start">
                  <span class="text-xs font-medium lg:ml-4">{{ relpy_question.username }}</span>
                </div>
                <div class="flex justify-between items-end">
                  <p class="text-sm lg:font-medium">
          ...
          <hx-form-item v-slot="{ field }" label="متن پاسخ" prop="content" :rules="[{
            required: true, message: ' متن پاسخ الزامی می باشد'
          }]" class="col-span-12">
            <hx-input show-word-limit maxlength="500" class="h-32" v-bind="field" type="textarea"
              v-model="answer_form.content" placeholder="متن پاسخ"></hx-input>
      ...
      <template #footer>
        <div>
          <hx-button :loading="loader" @click="submitAnswer()">ثبت پاسخ</hx-button>
        </div>
      </template>
    </hx-dialog>
  </div>
