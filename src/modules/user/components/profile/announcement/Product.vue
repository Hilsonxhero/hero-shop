      ...
      <div class="flex justify-end">
        <div class="">
          <hx-button
            icon
            variant="danger"
            @click="handleDeleteannouncement($event, announcement)"
          >
      ...
      <div class="relative flex items-center">
        <span class="flex-shrink-0 ml-3 w-28 h-28 block"
          ><img
            ...
            :src="announcement?.product?.media?.thumb"
          />
          ...
          <div class="flex items-end justify-end">
            ...
            >
              <div class="flex flex-col items-center justify-between">
                <span
                  v-if="
                    announcement?.variant?.is_incredible ||
                    announcement?.variant?.is_promotion
                  "
                  class="flex items-center text-sm text-typo-light line-through"
                >
                  <span class="">
                    {{ $filters.separate(announcement?.variant?.rrp_price) }}
                  </span>
                </span>
                <span class="flex items-center text-sm">
                  <span class="lg:font-bold lg:text-base">
                    {{
                      $filters.separate(announcement?.variant?.selling_price)
                    }}
                  </span>
...
const handleDeleteannouncement = (event, item) => {
  ...
  ApiService.delete(`user/profile/announcements/${item.id}`).then(
    ({ data }) => {
