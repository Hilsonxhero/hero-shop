  ...
  <div class="container my-12">
    <div class="grid grid-cols-12 gap-4">
      <div class="col-span-12 lg:col-span-9">
        <div class="bg-white shadow-lg lg:bg-transparent lg:shadow-transparent lg:border rounded-xl mb-4">
          <div class="p-5">
            ...
            <div class="mt-6">
              <section v-for="(payment_method, index) in payment_methods" :key="index">
                <hx-radio class="flex items-center bg-blue-100 p-4 cursor-pointer rounded-xl" :value="payment_method.id"
                  v-model="selected_method" :label="payment_method.id" name="payment_method">
                  <div class="flex flex-grow flex-wrap w-full items-center justify-between">
                    <h4 class="text-blue-600">{{ payment_method.title }}</h4>
                    <p class="text-sm text-gray-500 mr-2">
                      {{ payment_method.description }}
                    </p>
                    <hx-icon class="text-blue-600 h-8 w-8 hidden lg:block mr-2" icon="online-order"></hx-icon>
                  </div>
                </hx-radio>
              </section>
            </div>
          ...
          <div class="flex items-center justify-between p-5">
            ...
            <div class="flex items-center flex-wrap">
              <div class="flex items-center ml-3">
                <span class="ml-2">
                  <hx-icon class="w-6 h-6 text-gray-500" icon="envelope"></hx-icon>
                </span>
              ...
              <div class="flex items-center ml-3">
                <span class="ml-2">
                  <hx-icon class="w-6 h-6 text-gray-500" icon="mobile"></hx-icon>
                </span>
          ...
          <div class="w-full lg:block">
            <div class="relative">
              <div class="relative w-full h-10 rounded-xl">
                <input v-model="voucher_code" placeholder="کد تخفیف را وارد کنید" type="text"
                ...
                <div class="absolute inset-y-0 z-10 left-0 top-0 flex items-center justify-center w-16">
                  <div>
                    <button type="submit" class="cursor-pointer" @click="addVoucherCode">
                ...
                <div class="absolute inset-y-0 z-10 right-0 top-0 flex items-center justify-center w-16">
                  <div>
                    <hx-icon icon="gift" class="w-6 h-6 text-gray-500"></hx-icon>
                  </div>
                </div>
              </div>
            </div>
...
const addVoucherCode = () => {
  ...
  ApiService.post("voucher/check", formData)
    .then(({ data }) => {
      if (data.success) {
