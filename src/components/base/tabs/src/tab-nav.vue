<template>
  <div
    ref="el"
    :class="[ns.e('nav-wrap'), ns.is('scrollable', true), ns.is('top')]"
  >
    <div :class="[ns.e('nav-scroll')]" ref="navScroll">
      <div
        class="flex items-center space-x-reverse space-x-5"
        :class="[
          ns.e('nav'),
      ...
      >
        ...
        <div
          ...
          :ref="`tab-${pane.props.name}`"
          :class="[
            ns.e('item'),
            ns.is(rootTabs.props.tabPosition),
            ns.is('active', pane.active),
            ns.is('closable', closable(pane)),
          ]"
          :id="`tab-${pane.props.name}`"
          :key="`tab-${pane.props.name}`"
          :aria-controls="`pane-${pane.props.name}`"
          role="tab"
