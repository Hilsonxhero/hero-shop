...
import { useFormItem } from '@/core/hooks'
...
export const useSlide = (
  ...
  const handleSliderPointerEvent = (
    ...
    if (props.vertical) {
      ...
      const sliderOffsetLeft = slider.value!.getBoundingClientRect().right
      newPercent = ((clientX - Math.abs(sliderOffsetLeft)) / initData.sliderSize) * 100
      newPercent = Math.abs(newPercent)
    }
