<template>
    <div class="flex items-center">
        <div class="flex items-center">
            <input maxlength="1" autofocus @keyup.delete="previous"
                class="form-input  border-2 border-transparent focus:bg-white mr-2 h-10 focus:border-blue-500"
                type="text" v-for="(item, index) in length" v-model="values[index]" @input="next" placeholder="-" />
        </div>
        ...
        <div mode="out-in" class="flex flex-col justify-center mr-6 w-56">
            ...
            <div class="inline-block text-xs cursor-pointer text-typo-light font-medium opacity-20">
                ارسال مجدد
            </div>
        </div>
...
const next = (e: any) => {
    ...
    if (code.value.length < props.length) {
        ...
        e.target.nextSibling.nodeName == "INPUT" && value.trim().length >= 1 ? e.target?.nextSibling?.focus() : false
    }
}

const previous = (e: any) => {
    e.target.value = ''
    e.target.previousSibling.nodeName == "INPUT" ? e.target?.previousSibling?.focus() : false
}

watch(() => [...values.value], (curVal: any, prevVal: any) => {
